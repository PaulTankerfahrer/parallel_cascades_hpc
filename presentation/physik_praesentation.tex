...
\graphicspath{{stills/}{../results/images/}{./}}
...
\begin{frame}{Die Frage}
  Ein energiereiches Teilchen -- z.\,B.\ ein \alert{Neutron} aus einem
  Fusionsreaktor -- trifft einen Kristall. Was passiert im Material?
  \vfill
  \begin{itemize}
    \item Es schlägt ein Atom aus seinem Gitterplatz:
      das \alert{Primäre Rückstoßatom} (PKA).
    \item Dieses stößt Nachbarn an, die weitere anstoßen --
      eine \alert{verzweigende Kollisionskaskade}.
    \item Zurück bleibt ein \alert{Feld aus Gitterdefekten}.
      Über Jahre versprödet das Material.
  \end{itemize}
  \vfill
  {\small Relevanz: Wolfram als Wandmaterial in Fusionsreaktoren.
  Hier geht es nicht um ein konkretes Material, sondern um den
  \alert{Mechanismus} -- deshalb ein minimales, gut kontrollierbares 2D-Modell.}
\end{frame}
...
\begin{frame}{Das Modell}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      {\scriptsize Die beiden Kräfte: Feder (blau) + Abstoßung (rot).}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[standout]
  Was passiert, sieht man am besten in Bewegung.\\[0.4em]
  {\normalsize Vier Blicke auf dieselbe Kaskade.}
\end{frame}
...
\begin{frame}{Das physikalische Ergebnis: ein Potenzgesetz}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=\linewidth]{E2_powerlaw_overlay.png}\\
      {\scriptsize CCDF der Clustergrößen, log-log: eine Gerade über
...
% ---------------------------------------------------------------------
\begin{frame}[standout]
  Aus einem einzigen Stoß\\
  wird ein \alert{skalenfreies} Schadensmuster.
\end{frame}
